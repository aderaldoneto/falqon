...
function DynamicField({ field, value, onChange }: { field: PublicFormField; value: unknown; onChange: (value: unknown) => void }) {
  const configuration = field.configuration as Record<string, unknown>
  const choices = (configuration.choices ?? []) as Choice[]
  const label = `${field.label}${field.required ? ' *' : ''}`

  if (field.type === 'LONG_TEXT') {
    return <TextField fullWidth helperText={field.description} label={label} multiline minRows={4} onChange={(event) => onChange(event.target.value)} required={field.required} value={value ?? ''} />
  }
      ...
      <FormControl required={field.required}>
        ...
        <FormGroup>
          {choices.map((choice) => {
            ...
            return <FormControlLabel key={choice.value} control={<Checkbox checked={selected.includes(choice.value)} onChange={(_, checked) => onChange(checked ? [...selected, choice.value] : selected.filter((item) => item !== choice.value))} />} label={choice.label} />
          })}
        </FormGroup>
        {field.description && <FormHelperText>{field.description}</FormHelperText>}
      </FormControl>
    ...
    <Box minHeight="100vh" sx={{ background: 'radial-gradient(circle at 50% 0%, #282113 0%, #090a0d 42%)' }}>
      <Container maxWidth="sm" sx={{ py: { xs: 4, md: 8 } }}>
        ...
        {form.isError || !form.data ? (
          ...
          <Paper component="form" onSubmit={submit} variant="outlined" sx={{ mt: 4, p: { xs: 3, sm: 5 }, borderColor: alpha('#ffffff', 0.1), bgcolor: alpha('#121419', 0.94) }}>
            <Stack spacing={4}>
              <Box>
              ...
              {submitted ? (
                ...
                <>
                  {form.data.fields.map((field) => (
                    <DynamicField field={field} key={field.id} value={answers[field.id]} onChange={(value) => setAnswers((current) => ({ ...current, [field.id]: value }))} />
                  ))}
                  {submission.isError && <Alert severity="error">{submission.error.message}</Alert>}
                  <Button disabled={submission.isPending} size="large" type="submit" variant="contained">
