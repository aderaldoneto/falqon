        ...
        <Stack spacing={4}>
          ...
          {responses.isLoading ? (
            ...
            <Alert severity="error">
              {responses.error?.message ?? 'Formulário não encontrado.'}
            </Alert>
          ) : (
            <>
              <Box>
                ...
                <Typography component="h1" variant="h3" fontFamily="Georgia, serif" sx={{ mt: 1 }}>
                  {responses.data.title}
                </Typography>
                <Typography color="text.secondary" sx={{ mt: 1 }}>
                  {responses.data.submissions.length} resposta(s) recebida(s)
                </Typography>
              </Box>
              ...
              {responses.data.submissions.length === 0 ? (
                <Paper
                  variant="outlined"
                  sx={{ p: 6, textAlign: 'center', borderColor: alpha('#ffffff', 0.1) }}
                >
                ...
                responses.data.submissions.map((submission, index) => (
                  <Paper
                    key={submission.id}
                    variant="outlined"
                    sx={{ p: { xs: 3, sm: 4 }, borderColor: alpha('#ffffff', 0.1) }}
                  >
                    <Stack spacing={3}>
                      <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between">
                        <Typography fontWeight={800}>
                          Resposta #{responses.data!.submissions.length - index}
                        </Typography>
